...
const targetArt = {
  cliffwatch: "art-cliffwatch.png",
  advanced: "ui-building-card.png",
  grinning: "art-grinning.png",
  plinth: "art-plinth.png",
  aurora: "art-aurora.png",
  builder: "art-builder.png",
  harbor: "art-harbor.png",
  castle: "art-castle.png",
  blackstaff: "art-blackstaff.png",
  field: "art-field.png",
  entryWell: "art-entry-well.png",
  hallMirrors: "art-hall-mirrors.png",
  grimStatue: "art-grim-statue.png",
  hallVoice: "art-hall-voice.png",
  skullIsland: "art-skull-island.png",
...
const spaceDefs = {
  ...
  cliffC: { target: "cliffwatch", name: "崖望旅馆 C", detail: "刷新任务后拿 1 任务", effects: ["refresh", "quest"] },
  ...
  hallMirrors: { target: "hallMirrors", name: "镜厅", detail: "1 任意冒险者，或 1 盗贼 + 1 战士", effects: ["any", "r", "f"] },
...
function nextStep() {
  ...
  if (state.pending?.kind === "quickQuest") {
    ...
    addLog(`${state.pending?.source || "任务判定"}：已按实体桌面完成任务选择和补牌。`);
    finishQuickDecision();
    return;
  }
...
function applyShadowLocation(spaceId, options = {}) {
  ...
  if (target === "hallMirrors") addLog("镜厅：按实体任务需求选择 1 任意冒险者，或选择 1 盗贼 + 1 战士。");
  ...
  if (target === "skullIsland") addLog("骷髅岛：暗影领主获得 2 任意冒险者并获得 1 腐化；选择由你按实体任务短板判断。");
...
function addQuickQuest(source) {
  ...
  state.shadowNeedsQuest = false;
  addLog(`暗影领主按优先级从 ${source} 拿走 1 张任务；已记录为未知任务 #${state.quickQuestSerial}。`);
}
...
function completeQuickQuest(index, fusion) {
  ...
  const raw = window.prompt("这张任务牌奖励多少胜点？只填任务奖励里的胜点数字；其他奖励/费用请用资源按钮手动调整。", "0");
  if (raw === null) return;
  ...
  addLog(`暗影领主完成 ${questLabelFromEntry(entry)}，获得 ${gain} 胜点${fusion ? "（邪能融合翻倍一次）" : ""}。费用和非胜点奖励请按实体牌手动调整。`);
  render();
}
...
function handleAdvancedBuilding(index) {
  ...
  if (!item) return;
  if (!card) {
    addLog(`高级建筑：${item.name || "未指定建筑"}。请按实体牌面结算。`);
    return;
  }
  const tokens = parseTokens(card.staticResources);
  addResources(tokens);
  const ownerText = item.owner === "shadow" ? "暗影领主自己拥有，无拥有者收益。" : "这是你的建筑，别忘了你获得拥有者收益。";
  addLog(`高级建筑：${labelForCard(card)}。已自动加入可识别资源；其他牌面效果请按实体牌处理。${ownerText}`);
}
...
function renderQuestRecommendation(forceLog = false) {
  ...
  if (!result) {
    ...
    els.questCompletionBox.innerHTML = unknown
      ? `暗影领主有 ${unknown} 张手动记录任务；应用无法自动比对费用。若你判断可完成，请在任务卡片上点“普通完成”或“邪能融合完成”。`
...
function handleAction(action, target) {
  ...
  if (action === "quick-builder-bought") {
    addLog("建造者大厅：暗影领主已按实体桌面买下建筑；请把该建筑加入实体高级建筑区并补新建筑。");
    finishQuickDecision();
  }
...
function primaryActionConfig() {
  if (!state.configured) return { label: "按配置开始" };
...
function renderNextCard() {
  ...
  if (!state.configured) {
    ...
    els.nextCard.innerHTML = `
      <h3>先按实体桌面完成设置</h3>
      <p>选择模组后点击“按此配置开始”。应用会从第 1 轮的轮开始步骤接管流程提示。</p>
    `;
    return;
  }
...
function renderFinalScore() {
  ...
  els.finalScoreBox.innerHTML = `
    <div class="score-note">
      ${inlineResource("v")} 原版终局：分数轨 + 酒馆每个冒险者 + 每 ${inlineResource("g", 2)} = ${inlineResource("v")} + 领主卡奖励。
      暗影领主按 solo 规则没有领主卡，改为每张完成任务 ${inlineResource("v", 4)}。
    </div>
    ${isSkullport ? `
      <label class="score-global">
        ...
        <small>看腐化轨：离 -1 最远的空格数字；例如 -5 就填 5。玩家扣分，暗影领主按 solo 规则加分。</small>
      </label>
    ` : `
      <div class="score-note muted">${scoreIcon("res-corruption.png", "腐化")} 当前模组没有腐化终局计分。</div>
    `}
...
function scoreSideCard(side, title, total) {
  ...
  return `
    <div class="score-card">
      ...
      ${scoreLine(side, "vp", "当前分数轨", "res-vp.png", data.vp, `${data.vp || 0} VP`)}
      ${adventurerScoreLine(side, data, total.adventurers)}
      ${scoreLine(side, "g", "酒馆金币", "res-gold.png", data.g, `向下取整：${Math.floor((data.g || 0) / 2)} VP`)}
      ${isHuman
        ? scoreLine(side, "lord", "领主卡奖励", "icon-scroll.png", data.lord, "按领主卡牌面手填")
        : scoreLine(side, "completed", "完成任务张数", "icon-quest-completed.png", data.completed, `${data.completed || 0} x 4 = ${(data.completed || 0) * 4} VP`)}
      ${isSkullport
        ? scoreLine(side, "x", "腐化数量", "res-corruption.png", data.x, `${isHuman ? "-" : "+"}${(data.x || 0) * (state.finalScore.corruptionValue || 0)} VP`)
        : ""}
      ${scoreLine(side, "extra", "额外修正", "res-vp.png", data.extra, "特殊牌效或手动校正", { signed: true })}
...
function adventurerScoreLine(side, data, adventurerTotal) {
  return `
    <div class="score-line adventurer-line">
      <span class="score-label">${scoreIcon("res-any.png", "冒险者")} 酒馆冒险者</span>
      ...
      <small data-score-note="${side}-adventurers">每个 1 VP：${adventurerTotal} VP</small>
    </div>
  `;
}
...
function refreshFinalScoreNumbers() {
  ...
  updateScoreText('[data-score-note="human-adventurers"]', `每个 1 VP：${totals.human.adventurers} VP`);
  updateScoreText('[data-score-note="shadow-adventurers"]', `每个 1 VP：${totals.shadow.adventurers} VP`);
  updateScoreText('[data-score-note="human-g"]', `向下取整：${totals.human.gold} VP`);
  updateScoreText('[data-score-note="shadow-g"]', `向下取整：${totals.shadow.gold} VP`);
...
function renderActiveQuests() {
  if (!state.shadow.activeQuests.length) {
    els.activeQuestList.innerHTML = `<div class="rule-box">暗影领主没有进行中任务；下一次指派必须优先去崖望旅馆。</div>`;
    return;
  }
      ...
      if (!card) {
        return `
          <div class="quest-card quick-quest">
            ...
            <div class="meta">手动记录 · 具体费用和奖励由你按实体牌确认</div>
...
function renderAdvancedBuildings() {
  if (!state.advanced.length) {
    els.advancedBuildings.innerHTML = `<div class="rule-box">还没有场上高级建筑。暗影领主掷到高级建筑时会重掷，直到有建筑入场。</div>`;
    return;
  }
    ...
    .map((item, index) => {
      ...
      return `
        <div class="building-card">
          ...
          <div class="meta">${card ? `费用 ${inlineResource("g", card.cost ?? 0)} · ${setName(card.set)}` : "按实体牌面处理行动"}</div>
...
function addAdvancedBuilding() {
  ...
  addLog("已添加一个场上高级建筑位置；请按实体牌面处理行动。");
  render();
}
  ...
  return `
    <span class="rules-term" tabindex="0" role="button" aria-label="邪能融合说明">
      ...
      <span class="term-card" role="tooltip">
        <strong>邪能融合</strong>
        <span>把任意 2 个同类型冒险者转成 1 个其他类型冒险者，且转换必须帮助暗影领主完成任务。</span>
        <span>拿任务卡时可以预判是否本回合末可能靠它完成；真正执行转换仍在回合末任务检查时处理。</span>
        <span>快速算：先看金币，金币不足就不用算；再看是否能正常完成；最后逐个补缺口，每补 1 个缺口都要找 2 个同类型冒险者来换，并确认最终资源仍满足任务需求。</span>
        <span>用此法完成任务时，只把任务奖励中的胜点翻倍一次。</span>
      </span>
    </span>`;
}
